...
impl SimdComplex32 {
    ...
    pub fn double_inversion(two_complex: &[f32]) -> [f32; 4] {
    ...
    /// minuend: \[f32; 2\], subtrahend: \[f32; 4\]
    #[target_feature(enable = "simd128")]
    pub fn double_subtract(minuend: &[f32], two_subtrahends: &[f32]) -> [f32; 4] {
        let _minuend = unsafe { v128_load64_splat(minuend.as_ptr() as *const u64) };
        let _subtrahends = unsafe { v128_load(two_subtrahends.as_ptr() as *const v128) };
        let _result = f32x4_sub(_minuend, _subtrahends);
        unsafe { transmute(_result) }
    }

    /// base_term: \[f32; 2\], two_terms: \[f32; 4\]
    #[target_feature(enable = "simd128")]
    pub fn double_add(base_term: &[f32], two_terms: &[f32]) -> [f32; 4] {
